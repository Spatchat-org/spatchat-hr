...
def handle_chat(history, user_msg):
    ...
    if tool and tool.get("tool") == "home_range":
        if tool.get("method") == "mcp":
            mcp_list = tool.get("levels", [95])
        if tool.get("method") == "kde":
            kde_list = tool.get("levels", [95])
    if "mcp" in user_msg.lower():
        mcp_list = parse_levels_from_text(user_msg)
    if "kde" in user_msg.lower():
        kde_list = parse_levels_from_text(user_msg)
    if not (mcp_list or kde_list):
        mcp_list = [95]
    ...
    # build and return ZIP path
    zip_fp = save_all_mcps_zip()

    # prepare chat response
    msgs = []
    if mcp_list:
        msgs.append(f"MCPs ({','.join(map(str,mcp_list))}%) calculated.")
    if kde_list:
        msgs.append(f"KDEs ({','.join(map(str,kde_list))}%) calculated.")
