...
def clear_all_results():
    ...
    cached_df = None
    cached_headers = []
    # wipe outputs folder for a clean slate
...
def render_empty_map():
    ...
    for name, tile in [
        ...
        tile_args = {"tiles": tile} if tile.startswith("http") else {"tile": tile}
        folium.TileLayer(**tile_args, name=name).add_to(m)
        ...
        for aid in df.animal_id.unique():
            ...
            if pct not in mcp_results[aid]:
                ...
                if hull_pts is not None:
                    ...
                    # compute area in km² in WebMercator
...
def kde_home_range(lats,lons,pct,grid_size=200):
    # project to UTM, KDE, extract contour, return lat/lon polygon + area
    ...
    csum = np.cumsum(Z.ravel()[idx]*cell_area)
    ...
    def to_ll(poly):
        ex,ey = to_latlon.transform(*poly.exterior.xy)
        ints = [to_latlon.transform(*ring.xy) for ring in poly.interiors]
        return Polygon(list(zip(ex,ey)), [list(zip(ix,iy)) for ix,iy in ints])
    mpoly = to_ll(u) if isinstance(u,Polygon) else MultiPolygon([to_ll(p) for p in u.geoms])
    ...
    for pct in pct_list:
        for aid in df.animal_id.unique():
            ...
            if pct not in kde_results[aid]:
                ...
                if poly is not None:
                    # save GeoTIFF & GeoJSON
                    geojson_fp = os.path.join("outputs",f"kde_{aid}_{pct}.geojson")
                    with open(geojson_fp,"w") as f:
                        json.dump(mapping(poly), f)
                    tiff_fp = os.path.join("outputs",f"kde_{aid}_{pct}.tif")
                    # (skipping raster save here for brevity—add back your rasterio export if desired)
                    kde_results[aid][pct] = {"contour": poly, "area": area,
                                             "geojson": geojson_fp, "geotiff": tiff_fp}
...
def save_all_mcps_zip():
    ...
    # 3) Bundle everything under outputs/
...
def handle_upload_initial(file):
    ...
    try:
        ...
        return [], *(gr.update(visible=False) for _ in range(7)), render_empty_map()
    cached_df = df
    cached_headers = list(df.columns)
    # auto‐detect lat/lon
    lower = [c.lower() for c in df.columns]
    if "latitude" in lower and "longitude" in lower:
        return [{"role":"assistant","content":"CSV uploaded. Ready to compute home ranges."}], \
               *(gr.update(visible=False) for _ in range(3)), render_empty_map(), **{}
    else:
        return [{"role":"assistant","content":"Please upload a CSV with `latitude` & `longitude` columns."}], \
               *(gr.update(visible=False) for _ in range(3)), render_empty_map()

def parse_crs_input(c):
    c = str(c).strip().upper()
    if c.startswith("EPSG:"): return int(c.split(":")[1])
    if c.isdigit():      return 32600+int(c)
...
def handle_chat(history, user_msg):
    ...
    if re.search(r"\b(start over|restart|clear|reset)\b", user_msg, re.I):
        clear_all_results()
        return [{"role":"assistant","content":"All cleared! Upload a new CSV to begin."}], gr.update(value=render_empty_map()), ""
    # parse tool intent
    ...
    for name, tile in [
        ...
        args = {"tiles":tile} if tile.startswith("http") else {"tile":tile}
        folium.TileLayer(**args,name=name).add_to(m)
    # points
    ...
    for pct in requested_percents:
        ...
        for aid in df.animal_id.unique():
            ...
            if v:
                ...
                folium.Polygon(locations=coords, fill=True, color="red",
    ...
    for pct in requested_kde_percents:
        ...
        for aid in df.animal_id.unique():
            ...
            if v:
                ...
                folium.Polygon(locations=coords, fill=True, color="green",
                               fill_opacity=0.2).add_to(fg)
        fg.add_to(m)
    folium.LayerControl(collapsed=False).add_to(m)
    ...
    # build ZIP right now and return its path
