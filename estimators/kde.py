...
def kde_home_range(latitudes, longitudes, percent=95, animal_id="animal", grid_size=200):
    ...
    if total_prob > 0: Z_masked /= total_prob
    ...
    def utm_poly_to_latlon(poly):
        ...
        if isinstance(poly, Polygon):
            ...
            return Polygon(list(zip(ext_lon, ext_lat)), [list(zip(int_lon, int_lat)) for int_lon, int_lat in interiors])
    ...
    with rasterio.open(
        tiff_fp, "w", driver="GTiff",
        height=Z_masked.shape[0], width=Z_masked.shape[1],
        count=1, dtype=Z_masked.dtype, crs="EPSG:4326",
        transform=from_origin(lon_sw, lat_ne, (lon_ne - lon_sw) / grid_size, (lat_ne - lat_sw) / grid_size)
    ) as dst:
...
def add_kdes(df, percent_list):
    ...
    for percent in percent_list:
        for animal in df["animal_id"].unique():
            ...
            if percent not in storage.kde_results[animal]:
